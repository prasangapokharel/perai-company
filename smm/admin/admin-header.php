...
// Determine if we're in a subfolder (one level deep) or root admin
$isSubfolder = ($currentDir !== 'admin');
$baseUrl = $isSubfolder ? '../' : '';
?>
            ...
            <div class="p-6">
                ...
                <nav class="space-y-1">
                    ...
                    foreach ($navItems as $item):
                        $active = ($currentDir === str_replace('/', '', $item['url']));
                    ?>
                        <a href="<?php echo $baseUrl . $item['url']; ?>" class="admin-nav-item <?php echo $active ? 'active' : ''; ?>">
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-10 mb-4 px-4">Payments & Finance</p>
                <nav class="space-y-1">
                    <?php
                    $paymentItems = [
                        ['url' => 'oxapay-analytics.php', 'label' => 'OxaPay Analytics', 'icon' => '<path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />'],
                    ];
                ...
                <nav class="space-y-1">
                    ...
                    foreach ($contentItems as $item):
                        $active = ($currentPage === $item['url'] || $currentDir === str_replace('/', '', $item['url']));
                    ?>
                        <a href="<?php echo $baseUrl . $item['url']; ?>" class="admin-nav-item <?php echo $active ? 'active' : ''; ?>">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" <?php echo $item['icon']; ?>></path>
