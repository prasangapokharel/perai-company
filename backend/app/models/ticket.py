...
class Ticket(Base):
    ...
    ticket_opened_records = relationship(
        "TicketOpened", back_populates="ticket", cascade="all, delete-orphan"
    )
...
class TicketOpened(Base):
    ...
    ticket_id = Column(
        Integer, ForeignKey("ticket.id", ondelete="CASCADE"), nullable=False, index=True
    )
