...
def createTicket(
    ...
    category_str = ticket_data.category.lower() if hasattr(ticket_data.category, 'lower') else ticket_data.category
...
def updateTicket(
    ...
    if ticket_data.status is not None:
        ...
        status_str = ticket_data.status.lower() if hasattr(ticket_data.status, 'lower') else ticket_data.status
        if status_str in valid_statuses:
            ...
            if old_status == "open" and status_str == "closed":
                last_opened = db.query(TicketOpened).filter(
                    TicketOpened.ticket_id == ticket_id,
                    TicketOpened.closed_at == None
                ).first()
...
def getTicketOpenedRecords(
    db: Session,
    company_id: int,
    ticket_id: int
) -> list[TicketOpened]:
    """Get all ticket_opened records for a ticket."""
    return db.query(TicketOpened).filter(
        TicketOpened.company_id == company_id,
        TicketOpened.ticket_id == ticket_id
    ).order_by(TicketOpened.opened_at.desc()).all()
